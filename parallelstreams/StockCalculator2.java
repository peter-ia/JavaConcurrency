...
// Using parallel stream and explicit ForkJoinPool.
...
class StockCalculator2 {
	...
	public static void main(String[] args) {
		try {
			...
			ForkJoinPool threadPool = new ForkJoinPool(256);
			Future<Optional<Double>> future = threadPool.submit(() ->
				all
				.parallelStream()
				.map(StockCalculator2::getValue)
				.reduce((x, y) -> x + y)
			);
			
			Optional<Double> totalValue = future.get();
			...
			int parallelism = threadPool.getParallelism();
